...
export const validateAddress = async (request: Request, response: Response) => {
  try {
    const isOnCurve = PublicKey.isOnCurve((new PublicKey(request.query.address as string)).toBuffer());
    return response.status(200).send(isOnCurve);
  } catch (error) {
    return response.status(422).send(error.message);
  }
};
